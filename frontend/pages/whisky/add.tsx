  ...
  return (
    ...
    >
      ...
      <form className="p-3 space-y-3" onSubmit={addWhisky}>
        <input type="text" id="whiskyName" name="whiskyName" className="block w-full px-2 py-1 border border-gray-400 active:border-gray-800 focus:border-gray-800 outline-none" placeholder="name" onChange={(e) => (setName(e.target.value))}/>
        <input type="text" id="age" name="age" className="block w-full px-2 py-1 border border-gray-400 active:border-gray-800 focus:border-gray-800 outline-none" placeholder="age" onChange={(e) => (setAge(e.target.value))}/>
        <input type="number" id="abv" name="abv" className="block w-full px-2 py-1 border border-gray-400 active:border-gray-800 focus:border-gray-800 outline-none" placeholder="abv" onChange={(e) => (setABV(parseFloat(e.target.value)))}/>
        <input type="number" id="rating" name="rating" min="0" max="10" step=".1" className="block w-full px-2 py-1 border border-gray-400 active:border-gray-800 focus:border-gray-800 outline-none" placeholder="rating (0 or 1.0 - 10.0)" onChange={(e) => (setRating(parseFloat(e.target.value)))}/>
        ...
        <select id="characteristics" name="characteristics" multiple={true} className="block w-full px-2 py-1 border border-gray-400 active:border-gray-800 focus:border-gray-800 outline-none" onChange={(e) => (setCharacteristics(Array.from(e.target.selectedOptions, option => option.value)))}>
          {characteristics.map((characteristic) => (<option key={characteristic.id} value={characteristic.id}>{characteristic.name}</option>))}
        </select>
        <select id="tasteNotes" name="tasteNotes" multiple={true} className="block w-full px-2 py-1 border border-gray-400 active:border-gray-800 focus:border-gray-800 outline-none" onChange={(e) => (setTasteNotes(Array.from(e.target.selectedOptions, option => option.value)))}>
          {tasteNotes.map((tasteNote) => (<option key={tasteNote.id} value={tasteNote.id}>{tasteNote.name}</option>))}
        </select>
        <button type="submit" className="w-full wa-btn wa-btn-green">Add</button>
      </form>
    </div>
  );
};
